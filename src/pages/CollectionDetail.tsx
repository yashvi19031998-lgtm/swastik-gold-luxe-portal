    ...
    <main className="min-h-screen bg-background">
      ...
      <section className="relative pt-32 pb-16 overflow-hidden">
        <div className="absolute inset-0">
          <img src={collection.img} alt="" className="w-full h-full object-cover scale-110 blur-2xl opacity-30" />
          <div className="absolute inset-0 bg-gradient-to-b from-background/90 via-background/70 to-background" />
        </div>
        <div className="container-luxe relative grid md:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <Link to="/collection" className="text-xs tracking-[0.3em] uppercase text-gold-deep hover:text-primary transition-colors">
              ← All Collections
            </Link>
          ...
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1 }} className="relative">
            ...
            <img src={collection.img} alt={collection.name} loading="eager" className="relative rounded-3xl shadow-elegant w-full aspect-[4/3] object-cover" />
          </motion.div>
        </div>
      </section>
        ...
        <div className="container-luxe">
          {products.length === 0 ? (
            <div className="text-center py-24 glass rounded-3xl">
              ...
              <Link to="/shop" className="inline-flex mt-6 px-6 py-2.5 rounded-full bg-gradient-primary text-primary-foreground text-sm">Browse Shop</Link>
            </div>
          ) : (
      ...
      <section className="py-20 bg-secondary/40">
        <div className="container-luxe">
          <div className="flex items-end justify-between mb-10">
            ...
            <Link to="/collection" className="text-sm text-primary hover:text-gold-deep flex items-center gap-1">View all <ArrowRight className="w-4 h-4" /></Link>
          </div>
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-5">
            {related.map((c) => (
              <Link key={c.slug} to={`/collection/${c.slug}`} className="group relative rounded-2xl overflow-hidden shadow-card hover:shadow-elegant transition-all aspect-[4/5]">
