      ...
      <section className="relative pt-32 pb-20 overflow-hidden">
        ...
        <div className="container-luxe relative grid md:grid-cols-2 gap-12 items-center">
          <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            ...
            <Link href="/shop" className="mt-8 inline-flex items-center px-7 py-3 rounded-full bg-gradient-primary text-primary-foreground text-sm tracking-wide shadow-soft hover:shadow-elegant hover:-translate-y-0.5 transition-all">
              Explore Catalogue →
            </Link>
          </motion.div>
          <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1 }} className="relative">
            ...
            <Link href={`/collection/${featured.slug}`} className="block group">
              <img src={typeof featured.img === "string" ? featured.img : (featured.img as any).src} alt={featured.name} loading="eager" className="relative rounded-3xl shadow-elegant w-full aspect-[4/3] object-cover transition-transform duration-700 group-hover:scale-[1.02]" />
            ...
            {rest.map((c, i) => (
              ...
              >
                ...
                <div className="relative h-full flex flex-col justify-end p-6">
                  ...
                  <Link href={`/collection/${c.slug}`} className="mt-4 inline-flex items-center w-fit px-5 py-2 rounded-full bg-background/95 text-foreground text-xs tracking-wider hover:bg-gradient-gold hover:text-gold-foreground transition-all">
                    Explore Collection →
                  </Link>
                </div>
              </motion.div>
